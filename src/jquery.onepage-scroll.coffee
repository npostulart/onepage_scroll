...
wrap = ( $ ) ->
  ...
  $.onepage_scroll = ( element, options ) ->
    ...
    @createSections = ->
      ...
      $.each @sections, ( i, elem ) =>
        ...
        if @settings.pagination and !$(elem).hasClass @settings.noNavigationClass
  ...
  $.onepage_scroll::defaults =
    ...
    noNavigationClass: "no-navigation" # This option accepts any kind of class in case you wan't include single sections in pagination.
